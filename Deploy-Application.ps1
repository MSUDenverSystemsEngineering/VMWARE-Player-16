...
Try {
	...
	[string]$appName = 'Vmware Workstation Player '
	...
	[string]$installName = ''
	[string]$installTitle = 'VMware Workstation Player'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'vmplayer,vmware,vmnetshcp,vmware-authd,vmware-hostd,vmware-tray,vmware-usbarbitrator64,vmware-vmx,vmware-vmx-debug,vmware-vmx-stats,vmnetcfg,vmrest,containerd,containerd-shim-crx-v2,vctl' -CheckDiskSpace -PersistPrompt
		...
		$ExePath = Get-ChildItem -Path "$dirFiles" -Include VMware-player*.exe -File -Recurse -ErrorAction SilentlyContinue
		If($ExePath.Exists)
		{
		Write-Log -Message "Found $($ExePath.FullName), now attempting to install $installTitle."
		Show-InstallationProgress "Installing VMware Workstation Player. This may take some time. Please wait..."
		Execute-Process -Path "$ExePath" -Parameters "/s /v/qn EULAS_AGREED=1 AUTOSOFTWAREUPDATE=0 DATACOLLECTION=0 ADDLOCAL=ALL DESKTOP_SHORTCUT=0 REBOOT=ReallySuppress" -WindowStyle Hidden
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'vmplayer,vmware,vmnat,vmnetshcp,vmware-authd,vmware-hostd,vmware-tray,vmware-usbarbitrator64,vmware-vmx,vmware-vmx-debug,vmware-vmx-stats,vmnetcfg,vmrest,containerd,containerd-shim-crx-v2,vctl' -CloseAppsCountdown 60
		...
		## Uninstall Any Existing versions of VMware Workstation Player
		Remove-MSIApplications -Name 'VMware Player' -AddParameters 'REMOVE=ALL'
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+dor6XXr5xAj+koBj+TVI4za
# z1OgghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFJQWz6S5gPpglF7/PTNpZgcdpQP0MA0GCSqGSIb3DQEBAQUABIIBgGj+V4G5
# op6+71MyRJXoxOroUQvnRAvdIi6mZHQ6DzeMJxAR6q03w8QNpGSUK0CcYirfguIe
# YLd59LFUaYFCe7cCkDyQKZXRU1TCCYIS+Bf1ObIz3bTypfuA249WavYd8p+DFrMJ
# GXPvX95dkB3R9ncquqvXrrtQekrVAf8pTmtR0DXuqiiTdvGzkfyKPgoK966weycg
# nyE9eP1aLoCY7yqowjlFuflKM7SbMzZGXK6FfBoi9q4pDd6ZeYVVOickrIIHPYPO
# pMeNEsrTtaox09h7Wx5Z3e3FjFe5WN4Gh5E44EM0jztk/kFlandF6eHRgN7Nm/xh
# G0pnokx/aStLtp9d+I1GZZawtSpzpRZI1E5sci3JThoEwY0JKzViJQMflgKPrCHP
# 7HzNnQCfiPOIMuH9jE/VUeGRxWAJ8+le8vE2LY776/+7y7O1GNpD4MIL/eLuxseQ
# u6LTzSXGLzg8HQQ9STuB1pB0KP2dduIs9PtZlig9gyMOdHIPg+LA0arPbQ==
